...
@FeignClient(name = "AccountService", url = "http://localhost:8081/api")
public interface AccountClient {
    @GetMapping("/accounts/{id}")
    Optional<AccountResponse> findById(@PathVariable("id") Long id);

    @GetMapping("/accounts/search")
    Optional<List<AccountResponse>> findByAccountNumber(@RequestParam("accountNumber") Integer accountNumber);
